...
//Rare exceptions to indicate a bug in the _music code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_music = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_music]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.music.module"
